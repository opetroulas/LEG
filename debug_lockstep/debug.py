...
# 	["divideandconquer", rundir, start_pc, goal_pc, [js]] - Use the
# 		given run directory. Jump to the start_pc, jump or step instructions
#       as specified by the optional js string, then debug until reaching 
#       goal_pc. Finally, quit.
# 	["divideandconquer", rundir, start_pc, goal_pc, dump, dumpdir, [js]] - Use the
# 		given run directory. Jump to the start_pc, jump or step instructions
#       as specified by the optional js string, then dump the current processor 
#       state and start debuging until reaching goal_pc. Finally, quit.
...
def run_js(js):
	"""Jump and step instructions specified by a string before lockstepping
	Example string: 16,s12,1024 --> jump to pc=12, step 12 instructions, jump to address 1024"""
	cmds = js.split(',')
...
if TEST_FILE and COMMAND[0]=="autorun":
	...
	if("-js" in COMMAND): # Step or jump instructions before lockstepping
		i = COMMAND.index("-js")
		js = COMMAND[i+1]
		run_js(js)
	...
	print "    leg-stepm M: Step M instrucitons"
